...
export default {
    ...
    methods: {
        ...
        onInput(checkboxValue) {
            if (!this.fieldModel.includes(checkboxValue)) {
                this.fieldModel.push(checkboxValue);
            }
        },
        handleChangeEvent(selected){
            this.formModel[`${this.field.options.name}#inputValue`] = this.cleanObject(selected, this.formModel[`${this.field.options.name}#inputValue`])
        },
        cleanObject(selectedValues, obj) {
